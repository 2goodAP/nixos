{
  ...
  in {
    ...
    lspSignature.enable = mkEnableOption
      "Whether or not to enable lsp-signature.nvim";
  }

  config = let
    cfg = config.machine.programs.neovim;
  in lib.mkIf cfg.lsp.enable {
    machine.programs.neovim.startPackages = [
      pkgs.vimPlugins.nvim-lspconfig
      pkgs.vimPlugins.null-ls
    ] ++ (if cfg.lsp.lspsaga.enable
      then [pkgs.vimPlugins.lspsaga-nvim]
      else []
    ) ++ (if cfg.lsp.lspSignature.enable
      then [pkgs.vimPlugins.lsp_signature-nvim]
      else []
    );
